	...
	<form method="post" enctype="multipart/form-data" class="review-form">
		...
		<div class="input-wrapper">
			<div class="flex justify-between item-baseline py-10">
				<div class="rating">
					<input type="text" name="title" id="title" placeholder="리뷰 제목" class = "inputsize" value="${item.title}">
				</div>
				<div class="text-right"><input type="file" name="image" id="image" class = "picsize" multiple/><label for="image">사진 등록</label></div>
			</div>
			<div class="text-center py-15">
				<textarea class="contents" name="content" placeholder="게시글 변경" required>${item.content}</textarea>
			</div>
			<div class="text-center">
				<button class="btn gray round px-40 font-size-small" id="review-submit">게시글 변경</button>
			</div>
		</div>
	</form>
