...
<body>	
	...
	<div class="container">
		<h3 style="margin-top: 30px;">모임 수정</h3>
		<hr />
		<form role="form" id="registerForm" action="/club/add" method="post" enctype="multipart/form-data">
			...
			<div class="form-group">
				<label for="name"><strong>모임명</strong></label> <input type="text"
					class="form-control" id="name" name="name" value="${item.name}" required>
			</div>
			<div class="form-group">
				<label for="name"><strong>모임 요약</strong></label> <input type="text"
					class="form-control" id="intro" name="intro" value="${item.intro}" required>
			</div>
			<div class="form-group" style="width: 33%; float: left">
				<label for="startDate"><strong>시작일자</strong></label> <input
					type="date" class="form-control" id="startDate" name="startDate" value="${item.startDate}"
					required>
			</div>
			<div class="form-group" style="width: 33%; float: left;">
				<label for="startTime"><strong>시작시간</strong></label> <input
					type="time" class="form-control" id="startTime" name="startTime" value="${item.startTime}"
					required>
			</div>
			<div class="form-group" style="width: 33%; float: left;">
				<label for="endTime"><strong>종료시간</strong></label> <input
					type="time" class="form-control" id="endTime" name="endTime" value="${item.endTime}"
					required>
			</div>
			<div class="form-group">
			...
			<div class="form-group" id="formEndDate" hidden="true"
				...
				<label for="endDate"><strong>종료일자</strong></label> <input
					type="date" class="form-control" id="endDate" name="endDate" value="${item.endDate}">
			</div>
        ...
        <div class="form-group" style="clear: both">
            ...
            <textarea class="form-control" rows="5" id="content" name="content" required>${item.content}</textarea>
        </div>
        ...
        <div class="form-group" style="margin-top: 16px;">
            ...
            <input type="number" class="form-control" id="price" name="price" placeholder="원 단위로 숫자만 입력해주세요."  value="${item.price}">
        </div>
        <div class="form-group">
            <label for="membernum"><strong>모집 인원</strong></label>
            <input type="number" class="form-control" id="membernum" name="membernum" placeholder="최소 3명" value="${item.membernum}"required>
        </div>
