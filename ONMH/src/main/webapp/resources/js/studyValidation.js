...
function validation() {
    ...
    if($('#repeat').is(":checked")) {
        ...
        if(date1 === date2) {
            alert("정기모임에서 시작일자와 종료일자는 같을 수 없습니다.")
            return false;
        }else if(date1 > date2) {
    ...
    if($('#content').val() == "" || $('#content').val().replaceAll(" ", "").length == 0) {
        alert("상세정보를 입력해주세요");
        return false;
        ...
        let infoTemp = $('#content').val().trim().toString();
        infoTemp = removeTag(infoTemp);
        $('#content').val(infoTemp);
    }
